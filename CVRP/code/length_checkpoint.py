...
def eval_plot(n, bs, steps_per_epoch, n_epochs, opts, checkpoint_path):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_path', type=str, default='E:\\论文-毕业\\code\\checkpoints\\cvrp-500-6-cos.pt')
    ...
    # 得出model的结果
    checkpoint_dir =[
        'E:\\论文-毕业\\code\\checkpoints\\cvrp-500-1-cos.pt',
        'E:\\论文-毕业\\code\\checkpoints\\cvrp-500-3-cos.pt',
        'E:\\论文-毕业\\code\\checkpoints\\cvrp-500-5-cos.pt',
        'E:\\论文-毕业\\code\\checkpoints\\cvrp-500-6-cos.pt',
        'E:\\论文-毕业\\code\\checkpoints\\cvrp-500-7-cos.pt',
        'E:\\论文-毕业\\code\\checkpoints\\cvrp-500-8-cos.pt',
        'E:\\论文-毕业\\code\\checkpoints\\cvrp-500-9-cos.pt',
    ]
    best_obj = [51.345, 49.704, 45.613, 44.598, 43.221, 42.917, 42.2217]
    Avg_time = []

    # for i in range(7):
    #      best_avg_obj, avg_time = eval_plot(opts.problem_size, opts.batch_size, opts.steps_per_epoch, opts.n_epochs, opts, checkpoint_path = checkpoint_dir[i])
    #      best_obj.append(best_avg_obj)
    #      Avg_time.append(avg_time)
    #      print(f"Checkpoint {i} 已完成.")
    #      print(f"Best Average Objective: {best_avg_obj:.4f}, Average Time: {avg_time:.2f} seconds\n")
    print(best_obj)
    print(time)
    plt.figure(figsize=(8, 5))  # 设置图像大小
    plt.plot(range(7), best_obj, marker='o', linestyle='-', color='b', label="Best Objective")

    plt.xlabel("Checkpoint Index (i)")  # 设置横坐标标签
    plt.ylabel("Best Average Objective (Distance)")  # 设置纵坐标标签
    plt.title("Best Average Objective over Checkpoints")  # 设置标题
    plt.xticks(range(7))  # 让 x 轴显示整数
    plt.legend()  # 添加图例
    plt.grid(True)  # 添加网格
