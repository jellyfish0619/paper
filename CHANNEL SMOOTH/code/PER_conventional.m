function [packetErrorRate_sum]=PER_conventional(s, maxNumErrors, maxNumPackets, snr, seed_s) 
...
tgacChannel.TransmitReceiveDistance = 50; % Distance in meters for NLOS
...
%         到此结束探测信道的设置
%         探测信道估计
        chanEstSound = wlanVHTLTFChannelEstimate(vhtltfDemod_test,vhtSound);%得到信道估计 
...
%     得出转向矩阵T
%         T=Turn(V,NumStreams);
        T = permute(V(:,1:NumStreams,:),[3 2 1]); % Permute to Nst-by-Nsts-by-Nt

%         未平滑的BF信道的设置
         vhtBF = cfgVHT;
        vhtBF.SpatialMapping = 'Custom';
        vhtBF.SpatialMappingMatrix = T; 
        txPSDU=zeros(vhtBF.PSDULength*8,1);
      [numPacketErrors,numPkt]=PERTest(vhtBF,txPSDU,tgacChannel,maxNumErrors,maxNumPackets,packetSNR,ind,fs);


 packetErrorRate(i) = numPacketErrors/(numPkt-1);
 packetErrorRate_sum(i)=packetErrorRate_sum(i)+packetErrorRate(i);
  disp(['Non-smooth-BF:SNR ' num2str(snr(i)) ' completed after ' ...
